...
public interface IPasswordFieldFactory<T extends PasswordField, F extends IPasswordFieldFactory<T, F>> extends IFluentFactory<T, F>,
    IGeneratedVaadinPasswordFieldFactory<T, F, PasswordField, String>, HasSizeFactory<T, F>, HasValidationFactory<T, F>, HasValueChangeModeFactory<T, F>,
  ...
  // Protected override.
  default BooleanValueBreak<T, F> isRevealButtonVisible() {
    return new BooleanValueBreak<>(uncheckedThis(), get().isRevealButtonVisible());
  }

  // Protected override.
  default F setRevealButtonVisible(final boolean revealButtonVisible) {
